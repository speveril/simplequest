...
export class Map_Castle extends GameMap {
    ...
    open() {
        ...
        if (this.persisted('saw_lich_cutscene')) {
            ['lich','lackeyA','lackeyB','lackeyC','lackeyD'].forEach((n) => {
                let e = this.getAllEntitiesByName(n)[0];
                if (e) e.destroy();
            })
        }
    }
    ...
    start_boss_cutscene(args) {
        ...
        RPG.Scene.do(function*() {
            ...
            let d = Cozy.dist(RPG.getPlayer().position, target);
            RPG.getPlayer().behavior = RPG.Behavior['path'](RPG.getPlayer(), [[Math.atan2(target.y - RPG.getPlayer().position.y, target.x - RPG.getPlayer().position.x) * PIXI.RAD_TO_DEG, d],[270,0]]);
            yield *RPG.Scene.waitTime(d / RPG.getPlayer().speed + 0.1);
            RPG.getPlayer().sprite.animation = 'stand';
            ...
            lackeyA.behavior = RPG.Behavior['path'](lackeyA, [[ 90, 5 * RPG.getMap().tileSize.y],[  0, 0]]);
            lackeyB.behavior = RPG.Behavior['path'](lackeyB, [[ 90, 2 * RPG.getMap().tileSize.y],[ 90, 0]]);
            lackeyC.behavior = RPG.Behavior['path'](lackeyC, [[ 90, 2 * RPG.getMap().tileSize.y],[ 90, 0]]);
            lackeyD.behavior = RPG.Behavior['path'](lackeyD, [[ 90, 5 * RPG.getMap().tileSize.y],[180, 0]]);

            yield *RPG.Scene.waitTime(5 * RPG.getMap().tileSize.x / lackeyA.speed);

            lackeyB.hop(8);
            yield *RPG.Scene.waitTextbox('LACKEY', ["For the master!"]);
            lackeyB.speed = 128;
            lackeyB.behavior = RPG.Behavior['path'](lackeyB, [[ 90, RPG.getMap().tileSize.y]]);
            yield *RPG.Scene.waitTime(1 * RPG.getMap().tileSize.x / lackeyB.speed);
            yield *this.waitFight(lackeyB);

            lackeyC.hop(8);
            yield *RPG.Scene.waitTextbox('LACKEY', ["Death shall consume all!"]);
            lackeyC.speed = 128;
            lackeyC.behavior = RPG.Behavior['path'](lackeyC, [[ 90, RPG.getMap().tileSize.y]]);
            yield *RPG.Scene.waitTime(1 * RPG.getMap().tileSize.x / lackeyC.speed);
            yield *this.waitFight(lackeyC);

            lackeyA.hop(8);
            yield *RPG.Scene.waitTextbox('LACKEY', ["I might have a few regrets!"]);
            lackeyA.speed = 128;
            lackeyA.behavior = RPG.Behavior['path'](lackeyA, [[  0, RPG.getMap().tileSize.y]]);
            yield *RPG.Scene.waitTime(1 * RPG.getMap().tileSize.x / lackeyA.speed);
            yield *this.waitFight(lackeyA);

            lackeyD.hop(8);
            yield *RPG.Scene.waitTextbox('LACKEY', ["Die!"]);
            lackeyD.speed = 128;
            lackeyD.behavior = RPG.Behavior['path'](lackeyD, [[180, RPG.getMap().tileSize.y]]);
            yield *RPG.Scene.waitTime(1 * RPG.getMap().tileSize.x / lackeyD.speed);
            ...
            lich.speed = 128;
            lich.behavior = RPG.Behavior['path'](lich, [[ 90, 5 * RPG.getMap().tileSize.y]]);
            yield *RPG.Scene.waitTime(5 * RPG.getMap().tileSize.y / lich.speed);
