    ...
    export class Map_Boss extends SimpleQuest.Map {
        ...
        lowerPlatform() {
            RPG.Scene.do(function*() {
                ...
                Map.persistent[this.filename].platformHeight--;

                if (Map.persistent[this.filename].platformHeight > 0) {
                    this.platform.rect.y += this.tileSize.y;
                    this.dragon.adjust(0, this.tileSize.y);
        ...
        *dropSlimes(count:number = 1) {
        ...
        newSequence() {
            ...
            RPG.Scene.do(function*() {
                ...
                yield *this.dropSlimes(3 + (3 - Map.persistent[this.filename].platformHeight));
                RPG.sfx['dragon_roar'].play();
                yield *RPG.Scene.waitTime(1.0);

                this.dragon.sprite.animation = 'stand';

                var time = 0.5 + (Map.persistent[this.filename].platformHeight * 0.25);
                for (var i = 0; i < letters.length; i++) {
                    var letter = letters[i];
