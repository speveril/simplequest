...
export class Map_Cave extends GameMap {
    public static mapFile:string = 'map/cave.tmx';
    public static musicName:string = 'cave';
    ...
    entrance_cutscene() {
        if (!this.persisted('saw_cave_frontdoor_cutscene')) {
            ...
            RPG.Scene.do(function*() {
                ...
                koboldA.behavior = RPG.Behavior['path'](koboldA, [[ 270, 9 * RPG.getMap().tileSize.y ]]);
                koboldB.behavior = RPG.Behavior['path'](koboldB, [[ 180, 1.5 * RPG.getMap().tileSize.y ], [ 90, 0]]);
                yield *RPG.Scene.waitTime((1.5 * RPG.getMap().tileSize.y) / koboldB.speed);

                koboldB.hop(8);
                yield *RPG.Scene.waitTextbox("KOBOLD", ["You die now!"]);
                koboldB.behavior = RPG.Behavior['path'](koboldB, [[ 90, 3 * RPG.getMap().tileSize.y]]);
                yield *RPG.Scene.waitTime((3 * RPG.getMap().tileSize.y) / koboldB.speed);
