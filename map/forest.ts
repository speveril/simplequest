...
export class Map_Forest extends GameMap {
    ...
    cult_plateau_cutscene(args) {
        if (!this.persisted('plateau cutscene')) {
            ...
            RPG.Scene.do(function*() {
                yield* RPG.Scene.waitCameraMove(26.5 * RPG.getMap().tileSize.x, 18.5 * RPG.getMap().tileSize.y, 0.5);
                ...
                yield* RPG.Scene.waitCameraMove(RPG.getPlayer().position.x, RPG.getPlayer().position.y, 0.5);
            }.bind(this));
        }
    }
}